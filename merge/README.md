...
| `executor.py` | thin | T34 — the non-Claude-Code merge actor. Gathers signals, evaluates the gate (now including `review_verdict` — a `REQUEST_CHANGES` verdict is a hard block even with the label on), and — the load-bearing bit — **re-reads the head SHA and re-checks CI-green immediately before merging**, refusing on a moved head or non-green re-check. Merges via REST using the **`squash` method** (KGA-395: GitHub signs the server-side squash commit, so the default branch stays **Verified** and the coding agent's `unknown_key` feature-branch commits never reach it) with the `sha` param as a server-side backstop, then **deletes the merged head branch** so those unverified commits don't linger. |
| `revert.py` | pure + thin | T47 — post-merge verify + auto-revert. Polls CI on the merge commit (excluding its own run), and on red opens + **squash-merges** a revert PR (KGA-395: the fix lands Verified on `main`; the revert branch is then deleted), **idempotent on the merge SHA**. |
